	...
	SkipItIf(func() bool { return !helpers.RunsWithKubeProxy() },
		"Tests upgrade and downgrade from a Cilium stable image to master", func() {
			...
			assertUpgradeSuccessful, cleanupCallback =
				InstallAndValidateCiliumUpgrades(kubectl, helpers.CiliumStableHelmVersion, helpers.CiliumLatestHelmVersion, helpers.CiliumDevImage())
...
func InstallAndValidateCiliumUpgrades(kubectl *helpers.Kubectl, oldHelmVersion, newHelmVersion, newImageVersion string) (func(), func()) {
	canRun, err := helpers.CanRunK8sVersion(oldHelmVersion, helpers.GetCurrentK8SEnv())
	ExpectWithOffset(1, err).To(BeNil(), "Unable to get k8s constraints for %s", oldHelmVersion)
	if !canRun {
		Skip(fmt.Sprintf(
			"Cilium %q is not supported in K8s %q. Skipping upgrade/downgrade tests.",
			oldHelmVersion, helpers.GetCurrentK8SEnv()))
		return func() {}, func() {}
	}
	...
	ciliumCiliumState := func(helmPath, helmVersion string) {
		_ = kubectl.ExecMiddle("helm delete cilium --namespace=" + helpers.CiliumNamespace)
		ExpectAllPodsTerminated(kubectl)
		_ = kubectl.ExecMiddle(fmt.Sprintf("kubectl delete cm --namespace=%s cilium-config", helpers.CiliumNamespace))
		cmd := kubectl.CiliumHelm(
			"install",
			helmPath,
			"cilium",
			helmVersion,
			...
			map[string]string{
				...
				"agent.holdAfterClean": "true",
				"operator.enabled":     "false ",
			},
		)
		ExpectWithOffset(1, cmd).To(helpers.CMDSuccess(), "Cilium clean state %q was not able to be deployed", helmVersion)
		err = kubectl.WaitforPods(helpers.CiliumNamespace, "-l k8s-app=cilium", longTimeout)
		ExpectWithOffset(1, err).To(BeNil(), "Cilium %q was not able to be deployed", helmVersion)
		err = kubectl.WaitForCiliumInitContainerToFinish()
		ExpectWithOffset(1, err).To(BeNil(), "Cilium %q was not able to be clean up environment", helmVersion)
		cmd = kubectl.ExecMiddle("helm delete cilium --namespace=" + helpers.CiliumNamespace)
		ExpectWithOffset(1, cmd).To(helpers.CMDSuccess(), "Cilium %q was not able to be deleted", oldHelmVersion)
		ExpectAllPodsTerminated(kubectl)
	}
	...
	cleanupCallback := func() {
		...
		ciliumCiliumState(filepath.Join(kubectl.BasePath(), helpers.HelmTemplate), newHelmVersion)
	}
	...
	testfunc := func() {
		...
		cmd := kubectl.HelmRepoInstall()
		ExpectWithOffset(1, cmd).To(helpers.CMDSuccess(), "Unable to install helm repository")

		By("Cleaning Cilium state")
		ciliumCiliumState(filepath.Join(kubectl.BasePath(), "../test/cilium-1.6.6.tgz"), oldHelmVersion)

		By("Deploying Cilium %s", oldHelmVersion)
		cmd = kubectl.CiliumHelm(
			"install",
			filepath.Join(kubectl.BasePath(), "../test/cilium-1.6.6.tgz"),
			"cilium",
			oldHelmVersion,
			helpers.CiliumNamespace,
			nil,
		)
		ExpectWithOffset(1, cmd).To(helpers.CMDSuccess(), "Cilium %q was not able to be deployed", oldHelmVersion)
		...
		// kube-dns is running.
		By("Cilium %q is installed and running", oldHelmVersion)
		...
		By("Upgrading Cilium to %s", newHelmVersion)
		cmd = kubectl.CiliumHelm(
			"upgrade",
			filepath.Join(kubectl.BasePath(), helpers.HelmTemplate),
			"cilium",
		...
		ExpectWithOffset(1, cmd).To(helpers.CMDSuccess(), "Cilium %q was not able to be deployed", newHelmVersion)
		...
		By("Downgrading cilium to %s image", oldHelmVersion)
		...
		err = helpers.WithTimeout(
			waitForUpdateImage(oldHelmVersion),
